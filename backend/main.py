...
def execute(payload: dict[str, Any]):
    ...
    if kind in {"scale_up","scale_down","resize"}:
        current_i = int(service.get("instances", 1))
        if target is None:
            target = current_i + 1 if kind == "scale_up" else max(1, current_i - 1)
        else:
            target = int(target)
        ...
        service["cpu_percent"] = round(min(100.0, float(service["cpu_percent"]) * old/max(1,target)), 1)
