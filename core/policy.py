...
def validate_actions(service: dict, freshness: dict, candidates: list[dict], history: list[dict]) -> dict:
    ...
    for raw in candidates:
        ...
        if kind != "stop_service" and target is not None and (target < min_i or target > max_i):
        ...
        if reducing:
            ...
            if p > max_latency:
                ...
                continue
            if kind != "stop_service" and target < min_i:
        ...
        if kind == "stop_service":
            if rpm > 0:
                ...
                continue
            recent_rpm = [float(x.get("requests_per_minute", 0)) for x in history[-3:]]
            if any(v > 5 for v in recent_rpm):
                rejected.append({"action": kind, "reason": "idle gate: history shows recent traffic activity"})
                continue

        if kind == "scale_down":
            # Scale-down requires recent near-zero traffic evidence in history.
            recent_rpm = [float(x.get("requests_per_minute", 0)) for x in history[-3:]]
            if not recent_rpm or any(v > 5 for v in recent_rpm):
                rejected.append({"action": kind, "target_instances": target, "reason": "idle/capacity reduction requires recent near-zero traffic evidence in history"})
                continue
    ...
    # Prioritize SLA protection over cost reduction
    capacity.sort(key=lambda a: 0 if a.get("priority") == "SLA protection" else 1)
