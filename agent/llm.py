...
def deterministic_response(service, decision, execution, verification):
    ...
    if execution.get("recovered_from"):
        notes.append(f"Initial attempt failed ({execution['recovered_from'].get('error')}); recovered via smaller safe retry.")
    if verification.get("status") == "rollback_or_escalate" or not verification.get("latency_within_limit", True):
        notes.append(f"Post-action latency ({verification.get('after_latency_ms')}ms) remains outside target ({service.get('max_latency_ms',300)}ms); flagged for review.")
    ...
    return (f"{service.get('name','service')}: {decision.get('justification','No safe action available.')} "
            ...
            f"Verification={verification.get('status','unknown')}. Estimated hourly cost impact=${saving:.2f}.{note_str}")
